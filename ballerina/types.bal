...
// A single reasoning-action cycle of a paused agent run, in its Redis-storable form.
// `history` uses `ChatMessageDatabaseMessage` for the same reason `PendingApproval.history`
// does (see `ApprovalDatabaseMessage`); `output` narrows `ai:Error` to a `string` summary,
// since `ai:Error` values are not JSON-serializable.
...
// The Redis-storable form of `ai:PendingApproval`. Identical to `ai:PendingApproval` except
